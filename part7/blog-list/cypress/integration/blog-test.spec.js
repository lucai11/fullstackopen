...
Cypress.Commands.add('newBlog', ({ title, url }) => {
    cy.request({
        ...
        body: { title, url },
        headers: {
            'Authorization': `bearer ${JSON.parse(localStorage.getItem('blogLoginInfo')).token}`
        }
    })
})
...
describe('Blog App', function() {
    ...
    describe('when logged in', function() {
        ...
        it('can delete a blog', function() {
            ...
            cy.contains('the blogs title is....luna sucks wang chung').should('not.exist')
        })
        ...
        it.only('blogs are ordered by number of likes', function() {
            cy.newBlog({
                ...
                cy.newBlog({
                    ...
                    cy.reload()
                    for(let x=0; x<4; x++){
                        cy.contains(`backend attack${x+1}`).contains('view').click()
                        for(let y=0; y<x+1; y++){
                            cy.contains(`backend attack${x+1}`).contains('like').click()
                        }
                    }
                    cy.reload()
                    cy.get('.blog').then(blog => {
                        const likeOrder = blog.find('[data-cy="num-likes"]').map(function() {return this.firstChild.data})
                        for(let x=0; x<likeOrder.length-1; x++){
                            expect(Number(likeOrder[x])).to.be.greaterThan(Number(likeOrder[x+1]))
                        }
                    })
                })
            })
